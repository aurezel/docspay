...
function check_card(card_number) {
    var cardName = CardJs.cardTypeFromNumber(card_number);
...
!(function ($) {
    ...
    (CardJs.keyCodeFromEvent = function (e) {
        return e.originalEvent.data.charCodeAt(0);
    ...
    (CardJs.handleMaskedNumberInputKeyFF = function (e, mask) {
        ...
        if (isNumber || isDelete || isBackspace) {
            e.preventDefault();
            var newValue = rawText = $(element).val(),
                numbersOnly = CardJs.numbersOnlyString(rawText),
                digit = CardJs.digitFromKeyCode(keyCode),
                rangeHighlighted = normalisedEndCaretPosition > normalisedStartCaretPosition;
            if(rangeHighlighted) {
            ...
            if(caretStart != mask.length) {
                if(isNumber && rawText.length <= mask.length) {
                    numbersOnly = numbersOnly.slice(0, normalisedStartCaretPosition - 1) + digit + numbersOnly.slice(normalisedStartCaretPosition);
                    newCaretPosition = Math.max(CardJs.denormaliseCaretPosition(mask, normalisedStartCaretPosition + 1), CardJs.denormaliseCaretPosition(mask, normalisedStartCaretPosition + 2) - 1);
                }
            } else if(isDelete) {
                numbersOnly = numbersOnly.slice(0, normalisedStartCaretPosition) + numbersOnly.slice(normalisedStartCaretPosition + 1);
            } else if(0 != caretStart && isBackspace && !rangeHighlighted) {
    ...
    (CardJs.handleCreditCardNumberKeyFF = function (e, cardMask) {
        CardJs.handleMaskedNumberInputKeyFF(e, cardMask);
    }),
    ...
    (CardJs.handleExpiryKeyFF = function (e) {
        CardJs.handleMaskedNumberInputKeyFF(e, CardJs.EXPIRY_MASK);
    }),
    ...
    (CardJs.prototype.initCardNumberInput = function () {
        ...
        if(!isFirefox) {
            this.cardNumberInput.on('textInput', e => {
                CardJs.handleCreditCardNumberKey(e, $this.creditCardNumberMask);
            })
        } else {
            this.cardNumberInput.on('input', e => {
                CardJs.handleCreditCardNumberKeyFF(e, $this.creditCardNumberMask);
            })
        }
            this.cardNumberInput.keyup(function () {
                $this.refreshCreditCardTypeIcon();
            }),
            this.cardNumberInput.on("paste", function (e) {
    ...
    (CardJs.prototype.initCvcInput = function () {
        ...
        var $this = this;
        this.cvcInput.on('textInput', CardJs.filterNumberOnlyKey),
            this.cvcInput.on("paste", function (e) {
                var pasteData = e.originalEvent.clipboardData.getData('text');
                var that = $(this);
                setTimeout(function () {
                    that.val(pasteData);
    ...
    (CardJs.prototype.setupExpiryInput = function () {
        ...
        if (this.EXPIRY_USE_DROPDOWNS) {
            ...
            for (var expiryYearNew = $("<select><option value='any' selected='' hidden=''>YY</option></select>"), currentYear = parseInt(new Date().getFullYear().toString().substring(2, 4)), i = 0; i < CardJs.EXPIRY_NUMBER_OF_YEARS; i++)
                expiryYearNew.append("<option value='" + currentYear + "'>" + currentYear + "</option>"), (currentYear = (currentYear + 1) % 100);
            ...
            this.stripe && (this.expiryMonthInput.attr("data-stripe", "exp-month"), this.expiryYearInput.attr("data-stripe", "exp-year")),
            ...
            const isFirefox = navigator.userAgent.toLowerCase().includes('firefox');
            if(!isFirefox) {
                this.expiryMonthYearInput.on('textInput', e => {
                    ...
                    $this.EXPIRY_USE_DROPDOWNS || null == $this.expiryMonthYearInput || ($this.expiryMonthInput.val($this.expiryMonth()), $this.expiryYearInput.val(7 == val.length ? val.substr(5, 2) : null));
                })
            } else {
                this.expiryMonthYearInput.on('input', e => {
                    CardJs.handleExpiryKeyFF(e);
                    var val = $this.expiryMonthYearInput.val();
                    1 == val.length && parseInt(val) > 1 && CardJs.keyIsNumber(e) && $this.expiryMonthYearInput.val(CardJs.applyFormatMask("0" + val, CardJs.EXPIRY_MASK)),
                    $this.EXPIRY_USE_DROPDOWNS || null == $this.expiryMonthYearInput || ($this.expiryMonthInput.val($this.expiryMonth()), $this.expiryYearInput.val(7 == val.length ? val.substr(5, 2) : null));
                })
                ...
                this.expiryMonthYearInput.on("paste", function (e) {
                    var pasteData = e.originalEvent.clipboardData.getData('text');
                    var that = $(this);
                    setTimeout(function () {
                        that.val(pasteData);
    ...
    (CardJs.isExpiryValid = function (month, year) {
        ...
        return (
            2 == ("" + year).length && (year = currentYear.substring(0, 2) + "" + year),
